...
// SortStructByFloat
// filed 是结构体名字,不是tag 的名字
// s 是倒序标识。填入任意值，排序变倒序
func SortStructByFloat[T any](data []T, filed string, s ...any) []T {
	var (
		sortSlice    []float64
		sortStrSlice []string
	...
	if len(sortSlice) != 0 {
		sort.Float64s(sortSlice)
		if len(s) != 0 {
			for i := len(sortSlice); i > 0; i-- {
				out = append(out, val[fmt.Sprintf("%v", sortSlice[i-1])])
			}
		} else {
			for _, k := range sortSlice {
				out = append(out, val[fmt.Sprintf("%v", k)])
			}
		}
		return out
	}
...
// filed 是结构体名字,不是tag 的名字
// s 是倒序标识。填入任意值，排序变倒序
func SortStructByInt[T any](data []T, filed string, s ...any) []T {
	var (
		sortSlice    []int
		sortStrSlice []string
		val          = make(map[string]T)
