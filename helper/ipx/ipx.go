...
type iPCityResp struct {
	Country string `json:"country"`
	City    string `json:"city"`
}

func GetIPCity(ip string, lang ...string) string {

	var l = "zh-CN"
	if len(lang) > 0 && lang[0] != "zh" {
		l = lang[0]
	}
	url := fmt.Sprintf("http://ip-api.com/json/%s?lang=%s", ip, l)
	...
	return fmt.Sprintf("%s %s", resp.Country, resp.City)
}
